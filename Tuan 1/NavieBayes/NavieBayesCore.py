...
def naviebayes_main(list_stopword):
    #lấy list vocab
    ...
    #lấy list dict và số lượng các từ trong dict đó của mỗi label
    classification_dict = {}
    count_total = {}
        ...
        for label in classification_dict:
            for word in dict_word:
                if word not in classification_dict[label]:
                    ...
                    p += math.log10(1.0 + classification_dict[label][word]) / (len(vocabulary) + count_total[label]) * \
                         dict_word[word]
